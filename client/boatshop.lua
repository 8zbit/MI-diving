...
function SpawnBerthBoats()
    for loc,_ in pairs(QBBoatshop.Locations["berths"]) do
        if SpawnedBoats[loc] ~= nil then
            DeleteVehicle(SpawnedBoats[loc])
        end
    ...
    while true do
        ...
        if distance < 15 then
            ...
            if BuyDistance < 2 then                
                ...
                if not Buying then
                    DrawText3D(BuyLocation.x, BuyLocation.y, BuyLocation.z + 0.3, '~g~E~w~ - '..QBBoatshop.ShopBoats[currentBoat]["label"]..' bought for ~b~€'..QBBoatshop.ShopBoats[currentBoat]["price"])
...
AddEventHandler('qb-diving:client:BuyBoat', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    spawnvehicle(boatModel, function(veh)
